...
`intent.md` (card size, capacity, slot pitch). The parameters and feature tree below are
written against the *existing* geometry (base block + one ridge, no slots cut yet), which is
now fully parametric — both prior gaps (`Sketch001.Constraints[4]` → `BackHeight`,
`Pad.Length` → `LowerBackHeight`) were fixed by Bradley on 2026-09-06. Update this plan once
the open questions come in, then get explicit approval before any further FreeCAD execution.
...
| `BackHeight` | `App::PropertyLength` | 30mm | ✅ bound (`Sketch001.Constraints[4]`) — added by Bradley 2026-09-06 |
| `LowerBackHeight` | `App::PropertyLength` | 50mm | ✅ bound (`Pad.Length`) — added by Bradley 2026-09-06; overall base block height (Z) before the ridge feature |

All existing parametric gaps are closed — `audit_parametric.py` reports 0 issues as of
2026-09-06.
...
1. **Existing, fully parametric** — `Sketch` (`Width`×`Depth` rectangle) → `Pad` (base block,
   `LowerBackHeight`) → `Sketch001` (ridge profile, fully bound incl. `BackHeight`) →
   `Pad001` (ridge, fused).
2. Confirm card size/capacity (open questions in `intent.md`) → add `SlotCount`,
   `CardSlotClearance`, `WallBetweenSlots` to `Params.FCStd`.
3. Model one slot as a `PartDesign::Pocket` on a datum-plane-attached sketch (not a feature
   face — hard rule 3), dimensioned from `SlotHeight`, `SlotDepth`, `CardSlotClearance`.
4. `linear_pattern` the slot `SlotCount` times across `Width`, spaced by `SlotSpacing` +
   `WallBetweenSlots`.
5. Re-run `python3 scripts/audit_parametric.py` — must be clean.
6. Fillet/chamfer slot mouths per `CAD_STANDARDS.md` aesthetic guidance (avoid sharp overhangs
   for Silk filament).
7. Export STL/3MF to `stl/`/`3mf/` once print-ready.
...
* `python3 scripts/audit_parametric.py` must report 0 issues before any commit (current
  baseline: 0 issues, as of 2026-09-06).
...
* Bounding box must fit the Creality K2 Plus / ELEGOO Saturn 4 beds per `CAD_STANDARDS.md` —
  check once `Width`/`Depth`/`LowerBackHeight` are finalized (420mm width is worth
  double-checking against actual bed size before treating it as final).
...
Per `PROJECT_BOOTSTRAP.md`: **execution is forbidden until this plan is approved.** Steps 2–7
