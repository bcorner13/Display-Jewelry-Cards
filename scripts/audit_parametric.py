...
def audit_sketch(xml, name):
    ...
    # CORRECTED 2026-09-06 for this project: the canonical/Clocks regex
    # (`r'AttachmentSupport".*?<Link ...'`, DOTALL) keeps scanning past an EMPTY
    # AttachmentSupport (<LinkSubList count="0">) and matches the next <Link> found
    # anywhere later in the object body — which is often the sketch's unrelated
    # ExternalGeometry reference (e.g. a profile line snapped to a pad edge for
    # sizing, not an attachment). That produced a false-positive DAG-risk flag on
    # this project's Sketch001 (empty AttachmentSupport, but ExternalGeometry
    # references Pad.Face3). Fix: only look for a <Link> *inside* the
    # AttachmentSupport property's own span.
    attach = None
    asup = re.search(r'<Property name="AttachmentSupport"[^>]*>(.*?)</Property>', body, re.S)
    if asup:
        attach = re.search(r'<Link obj="(\w+)" sub="([^"]*)"', asup.group(1))
